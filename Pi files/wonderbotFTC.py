...
def main(config):
    ...
    while True: 
        if (GPIO.input(in_port) == GPIO.LOW):
            GPIO.output(out_ports, (GPIO.LOW,GPIO.LOW,GPIO.LOW))
            print("Port 32 was low number reset")
        ...
        if (val != 0):
            ...
            binnum = [int(i) for i in bin(val)[2:].zfill(3)]
            print("sending num ", binnum)
            GPIO.output(out_ports, binnum)
            dat = {"command":0}
